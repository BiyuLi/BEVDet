...
class CenterPointBBoxCoder(BaseBBoxCoder):
    ...
    def decode(self,
        ...
        if reg is not None:
            ...
            reg = reg.view(batch, self.max_num, 2)#FIXME:
        ...
        cam_box_ind = []
        for i in range(4):
            conf = self._transpose_and_gather_feat(cam_conf[i], inds)
            cam_conf_ind.append(conf)
            box = self._transpose_and_gather_feat(cam_box[i], inds)
            cam_box_ind.append(box)
        cam_conf_ind = torch.cat(cam_conf_ind,2)
        cam_conf_ind = cam_conf_ind.view(batch, self.max_num, 4)
        cam_box_ind = torch.cat(cam_box_ind,2)
        cam_box_ind = cam_box_ind.view(batch, self.max_num, 24)
        ...
        if vel is None:  # KITTI FORMAT
            ...
            # print(xs.shape)
            # print(cam_box_ind.shape)
            # print(cam_conf_ind.shape)
            # exit(0)
            final_box_preds = torch.cat([xs, ys, hei, dim, rot, vel,cam_conf_ind,cam_box_ind], dim=2)
