...
from hyper_dl.ops import bev_pool_v2
from mmdet.models.backbones.resnet import BasicBlock
from ..builder import NECKS
from tools.dataset_converters.lidar2cylinder import VirCylinderCamera
...
@NECKS.register_module()
class LSSCylinViewTransformer(LSSViewTransformer):

    def __init__(self, cam_config_path,cam_order, src_size, **kwargs):
        super(LSSCylinViewTransformer, self).__init__(**kwargs)
        self.frustum = self.create_cyl_frustum(kwargs['grid_config']['depth'], src_size,
                                            kwargs['input_size'], kwargs['downsample'])
        # 需要额外初始化四个虚拟的柱面相机，此处的初始化比较简易，
        with open(cam_config_path + 'cyl_camera.json', 'r') as file:
            cylinder_cfg = json.load(file)
        self.cfg = cylinder_cfg
        self.cfg["focal_length"] =self.cfg["focal_length"]/2
        self.cfg["cx"] = self.cfg["cx"] /2
        self.cfg["cy"]= self.cfg["cy"] /2
        with open(cam_config_path + 'cameras.json', 'r') as file:
            cam_cfg = json.load(file)
        self.cam_order = cam_order
        self.vir_cyls = {}
        for cam_type in self.cam_order:
            self.vir_cyls[cam_type] = VirCylinderCamera(cylinder_cfg)
            for i in range(4):
                if cam_cfg[i]['sensor']==cam_type:
                    self.vir_cyls[cam_type].init_local_camera(cam_type, cam_cfg[i]['lidar2cam_rotation'],
                                                cam_cfg[i]['lidar2cam_translation'])  # 所有相机的内参均一致

    def create_cyl_frustum(self, depth_cfg, src_size, input_size, downsample):
        """Generate the frustum template for each image.

        Args:
            depth_cfg (tuple(float)): Config of grid alone depth axis in format
                of (lower_bound, upper_bound, interval).
            src_size (tuple(int)): Size of origin images in format of (height,
                width).
            input_size (tuple(int)): Size of input images in format of (height,
                width).
            downsample (int): Down sample scale factor from the input size to
        ...
        if self.sid:
            ...
            d_sid = torch.exp(torch.log(depth_cfg_t[0]) + d_sid / (self.D-1) *
                              torch.log((depth_cfg_t[1]-1) / depth_cfg_t[0]))
            d = d_sid.view(-1, 1, 1).expand(-1, H_feat, W_feat)
        x = torch.linspace(0, W_ori - 1, W_feat,  dtype=torch.float)\
            .view(1, 1, W_feat).expand(self.D, H_feat, W_feat)
        y = torch.linspace(0, H_ori - 1, H_feat,  dtype=torch.float)\
            .view(1, H_feat, 1).expand(self.D, H_feat, W_feat)
    ...
    def get_lidar_coor(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
        ...
        B, N, _, _ = sensor2ego.shape
        for ii in range(4):
            r = sensor2ego[0,ii,:3, :3].T
            t = -sensor2ego[0,ii,:3, 3]
            r = r.cpu().numpy()
            t = t.cpu().numpy()
            self.vir_cyls[self.cam_order[ii]] = VirCylinderCamera(self.cfg)
            self.vir_cyls[self.cam_order[ii]].init_local_camera(self.cam_order[ii], r, t)

        # post-transformation
        # B x N x D x H x W x 3
        points = self.frustum.to(sensor2ego) - post_trans.view(B, N, 1, 1, 1, 3)  # self.frustum到cuda
        points = torch.inverse(post_rots).view(B, N, 1, 1, 1, 3, 3) \
            .matmul(points.unsqueeze(-1))  # 再去掉图像层面的数据增强的部分

        # img_to_ego
        points_list = []

        for i,cam_type in enumerate(self.cam_order):
            one_points = self.vir_cyls[cam_type].batch_convert_vir_camera_to_raw_camera(
                self.vir_cyls[cam_type].batch_convert_vir_img_to_vir_camera_by_vir_r(points[:,i:i+1,:,:,:,:]))  # img to vir to cam
            points_list.append(one_points)
        points = torch.cat(points_list,1)
        points[..., 0, :] = torch.clamp(points[..., 0, :], min=-51.2, max=51.2)
        points[..., 1, :] = torch.clamp(points[..., 1, :], min=-51.2, max=51.2)
   
        points = sensor2ego[:, :, :3, 3].view(B, N, 1, 1, 1, 3)+points.squeeze(-1)
        points = sensor2ego[:, :, :3, :3].view(B, N, 1, 1, 1, 3, 3).matmul(points.unsqueeze(-1)).squeeze(-1)  # cam to ego


        points = bda.view(B, 1, 1, 1, 1, 3,
                          3).matmul(points.unsqueeze(-1)).squeeze(-1)  # 同样去掉BEV层面的数据增强部分
        return points
